...
public class PlayerScript : MonoBehaviour
{
    ...
    private void TryLoadConfigFromPath(UnityWebRequest req, out PlayerConfig cfg)
    {
        ...
        {
            Debug.LogWarning("player_config.json parse failed. Using defaults. " + e.Message);
        }
        if (cfg == null)
        {
    ...
    private void ApplyConfig(PlayerConfig cfg)
    {
        ...
        if (spriteCount > 0)
        {
            ActiveSprite = Mathf.Clamp(cfg.PlayerSprite, 1, spriteCount) - 1;
        }
        else
        {
            ActiveSprite = 0;
        }
        moveForce = baseMoveForce * Mathf.Max(0.1f, cfg.SpeedMultiplier);
        jumpForce = baseJumpForce * Mathf.Max(0.1f, cfg.JumpMultiplier);
    }

    // Debug function to draw cube for groundDetect. Just for fun.
